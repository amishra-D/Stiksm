...
import { ScrollView, StatusBar, View, Text, TouchableOpacity, Animated, ActivityIndicator } from 'react-native';
...
export default function Quote() {
  ...
  const fetchQuotes = async () => {
    try {
      setLoading(true);
      const genAI = new GoogleGenerativeAI("AIzaSyC1JNXkeNAM01sWKScft6be1ZmLUznzWlg");
      const model = genAI.getGenerativeModel({ model: "models/gemini-1.5-pro-001" });
      
      const prompt = `Generate 4 short, single-line quotes in the style of ${categoryTitle.split(' Quotes')[0]}. Separate them with "||"`;
        ...
        >
          ...
          {quotes.length > 1 && (
            <View className="mt-8 border-t border-neutral-800 pt-6">
              <Text className="text-neutral-400 mb-4">More from {categoryTitle.split(' Quotes')[0]}</Text>
